...
def music_inference_model(LSTM_cell, densor, n_values = 78, n_a = 64, Ty = 100):
    ...
    inference_model = Model(inputs = [x0,a0, c0], outputs = outputs, name='InferModel')
...
def predict_and_sample(inference_model, x_initializer = x_initializer, a_initializer = a_initializer, 
    ...
    pred = np.array(inference_model.predict([x_initializer, a_initializer, c_initializer]))
    # Step 2: Convert "pred" into an np.array() of indices with the maximum probabilities
    indices = np.argmax(pred, axis=-1)
    # Step 3: Convert indices to one-hot vectors, the shape of the results should be (1, )
    results = to_categorical(indices, num_classes=pred.shape[-1])
    ### END CODE HERE ###
